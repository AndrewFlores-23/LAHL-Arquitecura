...
export default function Layout() {
  ...
  const [menuImage, setMenuImage] = useState(servicios[0].imagen);
  ...
  useEffect(() => {
    const onScroll = () => {
      ...
      setScroll({
        solid: y > 35,
        hidden: y > last.current && y > 220,
        floating: y > window.innerHeight * 0.7,
      });
      last.current = y;
    };
  ...
  return (
    <>
      ...
      <header
        className={`site-header ${scroll.solid || pathname !== "/" || open ? "solid" : ""} ${scroll.hidden && !open ? "hidden-header" : ""}`}
      >
        <Link
          className="brand"
          to="/"
      ...
      {open && (
        ...
        >
          <img
            ...
            src={asset(`images/${menuImage}`)}
            alt=""
          />
          <div className="menu-main">
            <p className="eyebrow">Explorá LAHL</p>
            <nav>
              {navegacion.map(([label, to], i) => (
          ...
          <div className="menu-services">
            <p className="eyebrow">Un servicio para cada etapa</p>
            {servicios.map((s) => (
              <Link
                key={s.slug}
                to={`/servicios/${s.slug}`}
                onClick={() => setOpen(false)}
                onMouseEnter={() => setMenuImage(s.imagen)}
                onFocus={() => setMenuImage(s.imagen)}
              >
                {s.nombre}
                <Arrow diagonal />
              </Link>
      ...
      {scroll.floating && !open && (
        ...
        >
          <svg
            ...
            aria-hidden="true"
          >
            <path d="M20.5 11.8a8.5 8.5 0 0 1-12.6 7.5L3 21l1.6-5A8.5 8.5 0 1 1 20.5 11.8Z" />
            <path d="M8 7.5c-2 3 4.5 9 7.5 7l1-2-3-1-.7 1c-1.5-.5-2.6-1.7-3-3l1-.7-1-2Z" />
          </svg>
        </a>
      )}
    </>
